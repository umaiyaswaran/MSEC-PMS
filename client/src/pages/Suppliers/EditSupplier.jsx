...
import { FormInput, Button, FileUpload, Loader } from '../../components';
...
const EditSupplier = () => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        ...
        setErrors({ fetch: err.response?.data?.message || 'Failed to load supplier' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: err.response?.data?.message || 'Failed to update supplier' });
    } finally {
      setSubmitting(false);
    }
  };

  if (loading) return <Loader message="Loading supplier data..." />;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">Update the supplier information below.</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add Documents (Optional)</h2>
          <FileUpload
        ...
        <div className="flex items-center justify-end space-x-4">
          <Button variant="outline" onClick={() => navigate(`/suppliers/${id}`)} disabled={submitting}>
            Cancel
          </Button>
