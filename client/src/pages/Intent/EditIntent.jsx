...
import { PRIORITY_LEVELS, PRIORITY_LABELS, INTENT_STATUSES } from '../../utils/constants';
...
const EditIntent = () => {
  ...
  useEffect(() => {
    const fetchIntent = async () => {
      try {
        ...
        const intentData = data.intent || data.data?.intent || data;

        if (intentData.status !== INTENT_STATUSES.DRAFT && intentData.status !== INTENT_STATUSES.REJECTED_BY_MANAGER) {
          setErrors({ submit: 'Only draft or rejected intents can be edited' });
          return;
        }
        ...
        setForm({
          ...
          department: intentData.department?._id || intentData.department || '',
          priority: intentData.priority || PRIORITY_LEVELS.MEDIUM,
          category: intentData.category || 'GOODS',
          items: intentData.items?.length > 0
            ...
            : [{ name: '', description: '', quantity: 1, unit: '', estimatedUnitPrice: 0, category: '' }],
        });
      } catch (err) {
        setErrors({ submit: 'Failed to load intent data' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const addItem = () => {
    setForm((prev) => ({
      ...prev,
      items: [...prev.items, { name: '', description: '', quantity: 1, unit: '', estimatedUnitPrice: 0, category: '' }],
    }));
  };
  ...
  const handleSubmit = async (isDraft = true) => {
    ...
    try {
      ...
      await intentApi.updateIntent(id, formData);
      navigate(`${navPrefix}/intents/${id}`);
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to update intent';
  ...
  if (loading) return <Loader message="Loading intent..." />;

  if (!intent && errors.submit) {
    return (
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Edit Intent</h1>
        <p className="mt-1 text-sm text-gray-500">
          Editing intent <span className="font-medium">{intent?.intentId}</span>
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-end space-x-4">
          <Button variant="outline" onClick={() => navigate(`${navPrefix}/intents/${id}`)} disabled={saving}>
            Cancel
          </Button>
