...
const Sidebar = ({ role, isOpen, onClose }) => {
  const prefix = role === 'admin' ? '/admin' : role === 'manager' ? '/manager' : role === 'store_manager' ? '/store-manager' : '';

  const menuItems = {
    user: [
      { label: 'Dashboard', path: '/user/dashboard', icon: 'home' },
      { label: 'My Intents', path: '/intents', icon: 'file-text' },
      { label: 'Create Intent', path: '/intents/create', icon: 'plus-circle' },
      { label: 'Notifications', path: '/notifications', icon: 'bell' },
      { label: 'Profile', path: '/settings/profile', icon: 'user' },
    ],
    manager: [
      { label: 'Dashboard', path: '/manager/dashboard', icon: 'home' },
      { label: 'Pending Approvals', path: '/manager/approvals', icon: 'clock' },
      { label: 'All Intents', path: prefix + '/intents', icon: 'file-text' },
      { label: 'Suppliers', path: '/manager/suppliers', icon: 'building' },
      { label: 'Purchase Orders', path: '/manager/purchase-orders', icon: 'shopping-cart' },
      { label: 'Notifications', path: prefix + '/notifications', icon: 'bell' },
    ],
    store_manager: [
      { label: 'Dashboard', path: '/store-manager/dashboard', icon: 'home' },
      { label: 'Open POs', path: '/store-manager/open-purchase-orders', icon: 'package' },
      { label: 'GRNs', path: '/store-manager/grns', icon: 'clipboard-check' },
      { label: 'Notifications', path: '/store-manager/notifications', icon: 'bell' },
      { label: 'Profile', path: '/store-manager/settings/profile', icon: 'user' },
    ],
    admin: [
      { label: 'Dashboard', path: '/admin/dashboard', icon: 'home' },
      { label: 'All Intents', path: prefix + '/intents', icon: 'file-text' },
      { label: 'Suppliers', path: prefix + '/suppliers', icon: 'building' },
      { label: 'Purchase Orders', path: prefix + '/purchase-orders', icon: 'shopping-cart' },
      ...
      { label: 'Departments', path: '/admin/settings/departments', icon: 'layers' },
      { label: 'Notifications', path: prefix + '/notifications', icon: 'bell' },
    ],
  };
  ...
  const icons = {
    ...
    'plus-circle': <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
    ...
    'shopping-cart': <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>,
    truck: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>,
    ...
    'clipboard-check': <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>,
  };
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 bottom-0 w-64 bg-sidebar z-50 transform transition-transform duration-200 ease-in-out lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-center h-16 border-b border-white/10">
          <h1 className="text-xl font-bold text-white">MSEC PMS</h1>
        </div>
